...
client.on(`message`, async (message) => {
  ...
  if(message.content.includes(client.user.id)) {
    message.reply(new Discord.MessageEmbed().setColor("#FFF800").setAuthor(`${message.author.username}, My Prefix is ${prefix}, to get started; type ${prefix}help`, message.author.displayAvatarURL({dynamic:true})));
  } 
...
//command Handler DO NOT TOUCH
 ...
 if (timestamps.has(message.author.id)) {
   ...
   if (now < expirationTime) {
     ...
     return message.reply(
      new MessageEmbed().setColor("#FFF800")
 ...
 try {
   ...
   console.error(error);
   message.reply( new MessageEmbed().setColor("#FFF800")
